...
### Level 5- DataRef Pattern ([#43](https://github.com/knative-extensions/educational-game/issues/43))

This level teaches the DataRef (Claim Check) pattern where large event payloads are stored in external storage and only a reference is passed through the eventing system. The player must click on the Storage Node to store the large payload, then route the generated reference box to the sink.
...
Other EDA Patterns like the Outbox Pattern will be designed and implemented in the future. The DataRef Pattern has been implemented in Level 5.
